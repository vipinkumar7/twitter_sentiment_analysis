...
public class MakeBatchDataSetFromCsv {
	...
	public MakeBatchDataSetFromCsv(String csvFileName, int batchSize, int ncol)
			throws Exception {
		this.iter = IOUtils.lineIterator(new FileInputStream(csvFileName),
				"UTF-8");
		this.ncol = ncol;
	...
	public DataSet makeBatch() {
		...
		for (int i = 0; i < batchSize; i++) {
			if (iter.hasNext()) {
				...
				// Deals with label
				target.putRow(i, Nd4j.create(new double[] { Double
						.parseDouble(arr[0]) }));
				// Deals with feature
